...
def extract_fields(cleaned_text):
    ...
    if surgery_record:
        ...
        procedure_match = re.search(
            r'手术经过、术中发现的情况及处理[：:](.*?)(?=平车|手术者签名|手术过程顺利|安返病房|手术者|$|$)',
            surgery_record,
            re.DOTALL
        )

        if procedure_match:
            extracted_text = clean_extracted_text(procedure_match.group(1))

            # 删除“患者姓名...组”这一段
            cleaned_text = re.sub(r'患者姓名.*?组', '', extracted_text, flags=re.DOTALL)
            fields_data['手术经过'] = cleaned_text
