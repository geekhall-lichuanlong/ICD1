...
def convert_info_docx(img, res, save_folder, img_name):
    ...
    doc.styles["Normal"].font.size = shared.Pt(10)  # 调整默认字号
    ...
    for i, region in enumerate(res):
        ...
        # 图片类型也应该处理其中的文本内容
        if region["type"].lower() == "figure" and isinstance(region["res"], list):
            ...
            for i, line in enumerate(region["res"]):
                if i == 0:
                    paragraph_format.first_line_indent = shared.Pt(20)
                if isinstance(line, dict) and "text" in line:
                    text_run = paragraph.add_run(line["text"] + " ")
                    text_run.font.size = shared.Pt(10)

    # docx_path = os.path.join(save_folder, "{}_ocr.docx".format(img_name))
    # 移除img_name中可能存在的文件扩展名
    base_name = os.path.splitext(img_name)[0]
    # 直接使用原文件名+.docx
    docx_path = os.path.join(save_folder, "{}.docx".format(base_name))
    doc.save(docx_path)
