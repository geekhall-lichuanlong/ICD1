...
def main():
    ...
    for idx, row in df.iloc[start_row:].iterrows():
        ...
        if all([
            ...
            # 计算所有字段的字符总数
            total_char_count = (
                len(case_id) + len(chief_complaint) + len(present_history) +
                len(past_history) + len(admission_status) + len(discharge_diag) +
                len(treatment_course) + len(course_record) + len(imaging_opinion) +
                len(us_result) + len(pathology_result) + len(preop_diag) + len(intraop_diag)
            )

            # 超过字符总数阈值则跳过
            if total_char_count > MAX_CHAR_COUNT:
                print(f"跳过第 {idx + 2} 行，病案标识: {case_id}，字符过长")
                continue
